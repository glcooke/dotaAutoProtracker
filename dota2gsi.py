...
class MyServer(HTTPServer):
    ...
    def handle_state(self, state):
        ...
        for handler in self.on_change_state:
            if "map" in state.keys():
                game_state = state["map"]["game_state"]
            else:
                game_state = "Main menu"

            if not self.last_state:
                last_game_state = "No state"
            elif "map" in self.last_state.keys():
                last_game_state = self.last_state["map"]["game_state"]
            else:
                last_game_state = "Main menu"
...
class Server:
    ...
    def start(self):
        ...
        if len(self.server.handlers) == 0 and len(self.server.on_ability_cast) == 0 and len(
                self.server.on_change_state) == 0:
    ...
    def on_state_change(self, func):
        self.server.on_change_state.append(func)